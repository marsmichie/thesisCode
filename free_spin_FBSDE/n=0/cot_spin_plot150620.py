...
def main(argv):
    ...
    for filename in os.listdir(folder):
        if filename.endswith(file_filter):
            ...
            ax1.plot(data.x,data.u*np.sin(data.x)/np.cos(data.x), "o", label= filename.replace("_"+file_filter,"").replace("m_",r'$\mu=$').replace("_n_",r', $\nu=$'))
            ax1.set_xlabel(r'$\vartheta$')
            ax1.set_title(r'osmotic velocities for $\nu=0$')
            ax1.set_ylabel(r'$\frac{\tilde{\Omega}^u_\vartheta(\vartheta)}{\cot(\vartheta)}$')
